...
busquedaRouter.get('/busqueda', (req, res) => {
  ...
  const sort = ((req.query.sort as string | undefined) || 'precio').trim();
  ...
  rows.sort((a, b) => {
    if (sort === 'cercania') return a.distanceKm - b.distanceKm;
    if (sort === 'balance') return a.balanceScore - b.balanceScore;
...
busquedaRouter.get('/familias/:familiaId/productos', (req, res) => {
  ...
  const grouped = new Map<string, { productName: string; minPrice: number; maxPrice: number; sellers: Set<string> }>();
  rows.forEach((row) => {
...
busquedaRouter.get('/productos/populares', (req, res) => {
  const search = ((req.query.search as string | undefined) || '').trim().toLowerCase();
  const limit = Number.parseInt((req.query.limit as string | undefined) || '12', 10) || 12;

  const grouped = new Map<string, { productName: string; score: number; minPrice: number; maxPrice: number; sellers: Set<string> }>();
  toSearchRows()
...
busquedaRouter.get('/productos/detalle', (req, res) => {
  const product = ((req.query.producto as string | undefined) || '').trim().toLowerCase();
