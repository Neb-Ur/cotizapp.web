...
const buildCatalogRow = (productoFerreteriaId: string) => {
...
catalogoFerreteriaRouter.post('/ferreterias/:ferreteriaId/catalogo', requireAuth, requireRole('ferreteria', 'admin'), (req, res) => {
  ...
  const exists = db.productosFerreteria.find(
    (item) => item.ferreteriaId === ferreteriaId && item.productoMaestroId === productoMaestro.id
  );
  if (exists) {
...
catalogoFerreteriaRouter.patch('/ferreterias/:ferreteriaId/catalogo/:productoFerreteriaId', requireAuth, requireRole('ferreteria', 'admin'), (req, res) => {
  ...
  const target = db.productosFerreteria.find(
    (item) => item.id === req.params.productoFerreteriaId && item.ferreteriaId === req.params.ferreteriaId
  );
  if (!target) {
  ...
  if (payload.skuFerreteria !== undefined) target.skuFerreteria = payload.skuFerreteria;
  if (payload.codigoBarras !== undefined) target.codigoBarras = payload.codigoBarras;
  if (payload.precio !== undefined) target.precio = payload.precio;
  if (payload.stock !== undefined) target.stock = payload.stock;
  if (payload.activo !== undefined) target.activo = payload.activo;
  if (payload.publicado !== undefined) target.publicado = payload.publicado;
...
catalogoFerreteriaRouter.delete('/ferreterias/:ferreteriaId/catalogo/:productoFerreteriaId', requireAuth, requireRole('ferreteria', 'admin'), (req, res) => {
  const index = db.productosFerreteria.findIndex(
    (item) => item.id === req.params.productoFerreteriaId && item.ferreteriaId === req.params.ferreteriaId
  );
  if (index < 0) {
    return fail(res, 'CATALOGO_NOT_FOUND', 'No existe el producto en el catalogo de la ferreteria.', 404);
