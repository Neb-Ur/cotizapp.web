...
taxonomiaRouter.post('/familias/:familiaId/atributos-definicion', requireAuth, requireRole('admin'), (req, res) => {
    ...
    const duplicated = db.definicionesAtributoFamilia.some((item) => item.familiaId === familiaId && item.codigo.toLowerCase() === payload.codigo.toLowerCase());
    if (duplicated) {
...
taxonomiaRouter.patch('/familias/:familiaId/atributos-definicion/:definicionId', requireAuth, requireRole('admin'), (req, res) => {
    ...
    if (payload.codigo !== undefined)
        target.codigo = payload.codigo;
    if (payload.etiqueta !== undefined)
        target.etiqueta = payload.etiqueta;
    if (payload.tipoDato !== undefined)
        target.tipoDato = payload.tipoDato;
    if (payload.esFiltrable !== undefined)
        target.esFiltrable = payload.esFiltrable;
    if (payload.esObligatorio !== undefined)
        target.esObligatorio = payload.esObligatorio;
    if (payload.opcionesJson !== undefined)
        target.opcionesJson = payload.opcionesJson;
    if (payload.orden !== undefined)
        target.orden = payload.orden;
