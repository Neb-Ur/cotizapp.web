...
const buildDetalle = (productoMaestroId) => {
...
productosMaestroRouter.patch('/productos-maestro/:productoMaestroId', requireAuth, requireRole('admin'), (req, res) => {
    ...
    if (payload.categoriaId !== undefined)
        target.categoriaId = payload.categoriaId;
    if (payload.subcategoriaId !== undefined)
        target.subcategoriaId = payload.subcategoriaId;
    if (payload.familiaId !== undefined)
        target.familiaId = payload.familiaId;
    if (payload.nombre !== undefined)
        target.nombre = payload.nombre;
    if (payload.marca !== undefined)
        target.marca = payload.marca;
    if (payload.descripcionCorta !== undefined)
        target.descripcionCorta = payload.descripcionCorta;
    if (payload.descripcionLarga !== undefined)
        target.descripcionLarga = payload.descripcionLarga;
    if (payload.imagenPrincipalUrl !== undefined)
        target.imagenPrincipalUrl = payload.imagenPrincipalUrl;
    if (payload.galeriaJson !== undefined)
        target.galeriaJson = payload.galeriaJson;
