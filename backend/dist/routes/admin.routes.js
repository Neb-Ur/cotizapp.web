...
adminRouter.patch('/admin/usuarios/:usuarioId', requireAuth, requireRole('admin'), (req, res) => {
    ...
    if (payload.planSuscripcion !== undefined)
        target.planSuscripcion = payload.planSuscripcion;
    if (payload.estadoCuenta !== undefined)
        target.estadoCuenta = payload.estadoCuenta;
    if (payload.nombre !== undefined)
        target.nombre = payload.nombre;
    if (payload.telefono !== undefined)
        target.telefono = payload.telefono;
    if (payload.ciudad !== undefined)
        target.ciudad = payload.ciudad;
    if (payload.comuna !== undefined)
        target.comuna = payload.comuna;
    if (payload.direccion !== undefined)
        target.direccion = payload.direccion;
